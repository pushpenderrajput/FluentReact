...
import Element from "../Components/FormsElements/Configue/Elements";
import { Field } from "../Components/FormsElements/Configue/types";
import { Button } from "@fluentui/react-components";




const DynamicForm: React.FC<{ formData?: any; data:any; }> = ({ formData, data}) => {
  ...
  return (
    <div style={{padding:"10px", boxShadow:"0px 0px 1.5px 1px #f3f3f3", borderRadius:"8px"}}>
    <h2>{data?.Parameter?.page_label}</h2>
    <form onSubmit={handleSubmit} style={{display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))", gap: "10px"}} >
      {fields.map((field) => (
         <Element
         key={field.field_id}
         field={field}
